describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    for (const [index, productItem] of prodItemsData.entries()) {
      console.log(`Checking product item ${index + 1}/${prodItemsData.length}`);
      if (!productItem.title || productItem.length === 0) { allArePopulated = false; }
      if (!productItem.title || productItem.length === 0) { allArePopulated = false; }
      if (!productItem.title || productItem.length === 0) { allArePopulated = false; }
    }
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    // Wait for 2.5 seconds to allow the button text to change
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      try {
        ...
        if (!shadowRoot) {
          console.warn(` No shadowRoot for product-item ${i}`);
          continue;
        }
        ...
        if (!button) {
          console.warn(` No button found in product-item ${i}`);
          continue;
        }
        ...
        if (buttonText === 'Add to Cart') {
          await button.click();
          console.log(` Clicked 'Add to Cart' on item ${i + 1}`);
        } else {
          console.warn(`Button text was '${buttonText}' for item ${i + 1}`);
        }

        await new Promise(r => setTimeout(r, 100)); // wait for cart to update

      } catch (err) {
        console.error(`Error clicking item ${i + 1}:`, err);
      }
    }

    const cartCountText = await page.$eval('#cart-count', el => el.innerText);
    console.log(`Cart count after clicking all: ${cartCountText}`);
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    for (const prodItem of prodItems) {
      const shadowRoot = await (await prodItem.getProperty('shadowRoot')).asElement();
      const button = await shadowRoot.$('button');
    ...
    const cartCountText = await page.$eval('#cart-count', el => el.innerText);
    console.log(`Cart count after removing all: ${cartCountText}`);
    expect(cartCountText).toBe('0');  // Cart should be empty now
}, 20000);
...
it('Checking number of items in cart on screen after reload', async () => {
  ...
  // Checking to make sure that the cart in localStorage is what you expect
